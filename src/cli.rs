...
pub fn cli() -> Option<LogLevel> {
    // the only config pass-able to the daemon via cli
    ...
    if let Some(command) = cli.command {
        ...
        println!(
            "{}",
            ::pandora::pithos::sockets::write_command_to_daemon_socket(&cmd)
                .expect("could not send command (is the daemon running?)")
        );
        process::exit(0);
    }
