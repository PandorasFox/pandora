...
impl Thread for WallpaperThreadHandle {
    ...
    fn start(&self, daemon: Weak<dyn Daemon + Sync + Send>, config: DaemonConfig) {
        ...
        match thread::Builder::new().name("wallpaper renderer".to_string())
            .spawn(move || {
...
impl WallpaperThread {
    ...
    fn scroll(&self, conn: &mut Connection<RenderThreadState>, state: &mut RenderThreadState, cmd: &ScrollCommand) {
        let output_name = cmd.output.clone();
        let output_state = match state.outputs.iter_mut().find(|(_, os)| os.name == output_name) {
            Some((_, os)) => os,
